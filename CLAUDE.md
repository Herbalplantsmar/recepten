...
1. **`app.js`** — the entire client. One IIFE that, on load, reads `document.body.dataset.page` and renders that page into `#app`. It also injects the shared nav (`#site-header`) and footer (`#site-footer`) into every page, so those are NOT in the HTML. Page renderers: `renderIndex` (gallery + search + left filter sidebar), `renderPlant` (`plant.html?plant=<id>`), `renderRecepten` (recipe photo-tile gallery + season filter; same card format as the plant grid), `renderReceptDetail` (full recipe at `recepten.html?recept=<id>`), `renderSeizoen`, `renderAdmin`. A recipe tile/hero photo is `recipe.afbeelding` or, as fallback, the first linked plant's `afbeelding` (`receptFoto()`); `img()` self-heals to the placeholder. The gallery `.grid` is fixed at **3 columns** (drops to 2/1 only on narrow screens). Static pages (`over`, `signup`, `diner`) get only nav+footer.
...
- `bronnen` — source URLs. Rendered on the detail page as a **numbered notes list** ("Bronnen"); the order of the array IS the footnote numbering (1 = first URL).
- **Inline citations**: prose fields carry footnote markers `[n]` (or `[n,m]` for several) placed directly after the claim they back, where `n` is the 1-based index into `bronnen`. `app.js` (`citeFrag`) turns each marker into a superscript link to the matching note. Markers are plain text inside the JSON strings — insert ONLY the bare `[n]`/`[n,m]` token, never alter surrounding wording/spacing. Fields that carry markers: `ecosysteem`, `gebruik`, `info`, `oogstmethode`, `wildplukGevolgen`, `verwisselingIntro`, `waarschuwing`, each `seizoenskalender[].toepassing`, and each `giftigeVerwisseling[].onderscheid`. `recept` (owner's voice) is left uncited. The validate hook rejects a marker whose number exceeds the plant's `bronnen` count.
